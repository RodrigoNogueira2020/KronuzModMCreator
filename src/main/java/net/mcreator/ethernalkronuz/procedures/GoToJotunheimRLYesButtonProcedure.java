...
public class GoToJotunheimRLYesButtonProcedure {
		...
		if (!((entity.level.dimension()) == (ResourceKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation("ethernal_kronuz:jotunheim"))))) {
			...
			if ((entity.level.dimension()) == (Level.OVERWORLD)) {
				...
				if (!world.isClientSide()) {
					...
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(new TextComponent("Dimension Before: Surface"), ChatType.SYSTEM, Util.NIL_UUID);
				}
			}
			if ((entity.level.dimension()) == (Level.NETHER)) {
				...
				if (!world.isClientSide()) {
					MinecraftServer _mcserv = ServerLifecycleHooks.getCurrentServer();
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(new TextComponent("Dimension Before: Nether"), ChatType.SYSTEM, Util.NIL_UUID);
				}
			}
			if ((entity.level.dimension()) == (Level.END)) {
				...
				if (!world.isClientSide()) {
					MinecraftServer _mcserv = ServerLifecycleHooks.getCurrentServer();
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(new TextComponent("Dimension Before: End"), ChatType.SYSTEM, Util.NIL_UUID);
				}
			}
			if ((entity.level.dimension()) == (ResourceKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation("ethernal_kronuz:asgard")))) {
				...
				if (!world.isClientSide()) {
					MinecraftServer _mcserv = ServerLifecycleHooks.getCurrentServer();
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(new TextComponent("Dimension Before: Asgard"), ChatType.SYSTEM, Util.NIL_UUID);
				}
			}
