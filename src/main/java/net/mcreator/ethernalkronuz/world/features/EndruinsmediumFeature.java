...
import net.mcreator.ethernalkronuz.procedures.EndruinsmediumAdditionalGenerationConditionProcedure;

import java.util.Set;
import java.util.List;

public class EndruinsmediumFeature extends Feature<NoneFeatureConfiguration> {
	public static EndruinsmediumFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new EndruinsmediumFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("ethernal_kronuz:endruinsmedium", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("ethernal_kronuz:endruinsmedium", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if ((context.random().nextInt(1000000) + 1) <= 5000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!EndruinsmediumAdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
