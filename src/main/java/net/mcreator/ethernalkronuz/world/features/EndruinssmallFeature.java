...
import net.mcreator.ethernalkronuz.procedures.EndruinssmallAdditionalGenerationConditionProcedure;

import java.util.Set;
import java.util.List;

public class EndruinssmallFeature extends Feature<NoneFeatureConfiguration> {
	public static EndruinssmallFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new EndruinssmallFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("ethernal_kronuz:endruinssmall", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("ethernal_kronuz:endruinssmall", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if ((context.random().nextInt(1000000) + 1) <= 5000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!EndruinssmallAdditionalGenerationConditionProcedure.execute(world, x, y, z))
					continue;
